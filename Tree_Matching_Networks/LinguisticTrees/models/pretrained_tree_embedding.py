...
# models/pretrained_tree_embedding.py
...
class PretrainedTreeEmbeddingNet(GraphEmbeddingNet):
    """GNN propagation + pretrained HF transformer aggregator (embedding mode).

    Used for Conditions D, E, F.
    Same forward interface as TreeEmbeddingNet (via GraphEmbeddingNet).
    """
    ...
    def __init__(self, config):
        ...
        print(f"PretrainedTreeEmbeddingNet initialized:")
    ...
    def freeze_transformer(self):
        """Freeze pretrained transformer in aggregator (Condition E)."""
        self._aggregator.freeze_transformer()

    def unfreeze_transformer(self):
        self._aggregator.unfreeze_transformer()
    ...
    def get_parameter_groups(self, base_lr, pretrained_lr_scale=0.1):
        ...
        for name, param in self.named_parameters():
            if not param.requires_grad:
                continue
            if '_aggregator.encoder.' in name:
