...
# models/pretrained_noprop_matching.py
import torch.nn as nn
from ...GMN.pretrained_transformer_aggregator import PretrainedTransformerAggregator


class PretrainedNoPropMatchingNet(nn.Module):
    """Condition B matching: Raw node features -> pretrained HF -> graph embedding.

    No GNN propagation. The aggregator processes each graph independently;
    comparison happens in the loss function (matching paradigm without
    cross-graph attention during propagation, since there is no propagation).
