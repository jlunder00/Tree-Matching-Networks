...
def convert_tree_to_graph_data(tree_pairs: List[Dict]) -> GraphData:
    ...
    return GraphData(
        ...
        node_features=torch.tensor(all_node_features, dtype=torch.float32).detach(),
        edge_features=torch.tensor(all_edge_features, dtype=torch.float32).detach(),
